...
class ElDragDiv extends Component {
    ...
    render() {
        const { isDragging, connectDragSource, id, elementSelectedStyle } = this.props;
        if (isDragging) {
            ...
            return connectDragSource(
                <div className={`${styles.right} ${styles[elementSelectedStyle]}`} id={id}>{this.props.children}</div>
            );
        }
    }
}
