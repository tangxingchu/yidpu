...
const selectQueueById = (id) => {
    return (dispatch, getState) => {
        dispatch({type: Queue.SELECT_QUEUE_PENDING});
...
const updateTableStatus = (data) => {
    return (dispatch, getState) => {
        return Promise.resolve(
            dispatch({type: Queue.UPDATE_TABLE_STATUS_SUCCESS, payload: data})
        );
    }
}

const deleteQueueElementById = (id, queueId) => {
    return (dispatch, getState) => {
        return Promise.resolve(
            dispatch({type: Queue.DELETE_QUEUE_ELEMENT_SUCCESS, payload: {id, queueId}})
        );
    }
}
