...
Graph.prototype.init = function(container)
{
	...
	if(Graph.fileSupport) {
		this.initImageDrop(container);
	}
};

//添加能够支持拖拽的方法
Graph.prototype.initImageDrop = function(container) {
	...
	mxEvent.addListener(container, 'drop', function(evt)
	{
		if (sb.isEnabled())
		{
			...
			var pt = mxUtils.convertPoint(sb.container, mxEvent.getClientX(evt), mxEvent.getClientY(evt));
			var tr = sb.view.translate;
			var scale = sb.view.scale;
			...
			{
				sb.handleDrop(filesArray[i], x + i * 10, y + i * 10);
			}
		}
	});
}

Graph.prototype.handleDrop = function(file, x, y) {
	var sb = this;
			...
			if (file.type.substring(0, 9) == 'image/svg')
			{
				...
				if (root != null)
				{
					...
					if (svgs.length > 0)
					{
						...
						sb.insertVertex(null, null, '', x, y, w, h, 'shape=image;image=' + data + ';');
					}
				}
			}
			else
...
Graph.prototype.getTooltipForCell = function(cell)
{
	...
	if (mxUtils.isNode(cell.value))
	{
		...
		{
			var ignored = ['label', 'tooltip', 'placeholders', 'placeholder', 'tableId', 'tableLimit'];
