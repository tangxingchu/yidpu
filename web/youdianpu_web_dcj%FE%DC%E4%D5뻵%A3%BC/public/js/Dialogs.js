...
var AboutDialog = function(editorUi)
{
	...
	mxUtils.write(h3, mxResources.get('about'));
	div.appendChild(h3);
...
var EditDataDialog = function(ui, cell)
{
	...
	//console.log(cell.getAttribute('p1'));
	...
	for (var i = 0; i < attrs.length; i++)
	{
		if (attrs[i].nodeName != 'label' && attrs[i].nodeName != 'placeholders' 
			&& attrs[i].nodeName != 'tableId' && attrs[i].nodeName != 'tableLimit')
		{
			var name = attrs[i].nodeName;
			// if (name == 'tableCode') {
			// 	name = name + '(桌台编号)';
			// }
			temp.push({name: name, value: attrs[i].nodeValue});
		}
	}
	...
	var addBtn = mxUtils.button(mxResources.get('addProperty'), function()
	{
		...
		if (name.length > 0 && name != 'label' && name != 'placeholders' 
			&& name != 'tableId' && name != 'tableLimit' && name.indexOf(':') < 0)
		{
			try
			{
 ...
 * Constructs a new link dialog.
 */
var DropImageSizeDialog = function(editorUi, initialValue, btnLabel, fn)
{
	var div = document.createElement('div');
	mxUtils.write(div, mxResources.get('editLink') + ':');
	...
	var widthInput = document.createElement('input');
	widthInput.setAttribute('value', initialValue);
	widthInput.setAttribute('placeholder', '请输入图片的高');
	widthInput.setAttribute('type', 'text');
	widthInput.style.marginTop = '6px';
	widthInput.style.width = '400px';
	widthInput.style.backgroundImage = 'url(\'' + Dialog.prototype.clearImage + '\')';
	widthInput.style.backgroundRepeat = 'no-repeat';
	widthInput.style.backgroundPosition = '100% 50%';
	widthInput.style.paddingRight = '14px';
