...
const listCartExtra = (goodsIds) => {
    return (dispatch, getState) => {
        dispatch({type: FloorPlan.QUERY_CARTEXTRA_PENDING});
        return requestapi({uri: `/api/floorPlan/listCartExtra`, fetchParams: {body: {goodsIds}}}).then((data) => {
            dispatch({type: FloorPlan.QUERY_CARTEXTRA_SUCCESS, payload:  data });
        }).catch(err => {
            dispatch({type: FloorPlan.QUERY_CARTEXTRA_FAILURE, payload: err.message});
            throw err;
        });
    }
}
...
//确认订单项
const confirmOrderItem = (orderNo, orderItemId) => {
    return (dispatch, getState) => {
        ...
        return requestapi({uri: `/api/order/confirmOrderItem`, fetchParams: {
            ...
            body : `orderNo=${orderNo}&orderItemId=${orderItemId}`,
        }}).then((data) => {
