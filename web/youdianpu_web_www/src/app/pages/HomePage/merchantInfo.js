...
class MerchantInfo extends Component {
    ...
    onChange_logo = ({ file, fileList }) => {
        ...
        this.props.logoOnChange(fileList);
    }
    ...
    render() {
        const { photoList = [], yyzzList = [], logoList = [], form, saveLoading, auditHisLoading } = this.props;
        ...
        let logoError = null;
        if (this.props.logo.errors) {
            logoError = this.props.logo.errors[0].message;
        }
        let yyzzError = null;
            ...
            <Card bordered={false}>
                ...
                <Form >
                    ...
                    <FormItem {...formItemLayout} label="店铺logo"
                        help={logoError ? logoError : '请上传您的店铺logo,不能超过1M'}
                        validateStatus={logoError ? 'error' : ""}
                    >
