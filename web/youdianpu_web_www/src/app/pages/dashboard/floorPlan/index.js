...
import { Modal, Affix, Radio, message, notification, Collapse } from 'antd';
...
class FloorPlan extends Component {
    ...
    componentDidMount() {
        ...
        if (screenList.length === 0) {
            this.props.screenActions.list({}).then((floors) => {
                this.getFloorXml(floors);
            }).catch(e => message.error(e));
        } else {
            this.getFloorXml(screenList);
        }
        //下单
 ...
 */
    ...
    listTodayGoodsDayDiscount = (goodsIds) => {
        ...
        if(isEnabledDiscount == "0" || isEnabledDays == "0") {
            ...
            if(goodsIds && goodsIds.length > 0) {
                this.props.floorPlanActions.listCartExtra(goodsIds);
    ...
    submitOrder = (dinersNum) => {
        ...
        this.props.floorPlanActions.submitOrder(cart).then((results) => {
            ...
            ipcRenderer.on('selectPrintConfigs-reply', (event, arg) => {
                if(arg) {
                    ...
                    if(autoPrintTicket) {
                        ...
                        ipcRenderer.send('printTicket', {cart: {...cart, ...results}, merchant: currMerchantInfo});
                    }
                    ...
                    if(autoPrintOrder) {
                        ...
                        ipcRenderer.send('printOrder', {cart: {...cart, ...results}});
                    }
                }
            ...
            */
            /*
            const autoPrintTicket = window.localStorage.getItem('autoPrintTicket');
            if(autoPrintTicket == 1) {
                ipcRenderer.send('printTicket', {cart: {...cart, orderNo}, merchant: currMerchantInfo});
            } else {
                Modal.confirm({
                    title: '提示',
                    content: '用餐订单已成功提交，是否打印小票？',
                    okText: "是",
                    cancelText: '否',
                    onOk: () => {
                        ipcRenderer.send('printTicket', {cart: {...cart, orderNo}, merchant: currMerchantInfo});
    ...
    confirmOrder = (orderNo) => {
        ...
        this.props.floorPlanActions.confirmOrder(orderNo).then(() => {
    ...
    confirmOrderItem = (orderNo, orderItemId) => {
        const { orderList } = this.props.floorPlan;
        const data = orderList[0];
        this.props.floorPlanActions.confirmOrderItem(orderNo, orderItemId).then((result) => {
    ...
    render() {
        ...
        const { defaultFloorId, graphLeft, local_goodsList, local_cartList, local_goodsCategory, goodsLoading, 
            ...
            countDown, selectOrderLoading, confirmOrderLoading, paymentFormData } = this.props.floorPlan;//默认场地id, graph靠左边距, 本地商品信息
