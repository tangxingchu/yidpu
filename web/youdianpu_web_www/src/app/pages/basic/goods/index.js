...
class Goods extends Component {
    ...
    render() {
        ...
        const { loading, goodsList, pageSize, total, currentPage, goodsData, saveLoading, fileList, extraList, extraItems, selectedExtraItems,
            dictItemMap = {}, activeKey } = this.props.goods;
        const { categoryList } = this.props.goodsCategory;
        const { fieldChangeValue, fileOnChange, resetFields } = this.props.goodsActions;
        const { modalVisible, name, status, categories } = this.state;
        ...
        return (
            <PageHeaderLayout
                title="商品管理"
                content="商品管理是对各类商品的名称、价格以及库存管理。能方便客户在移动设备快速下单。"
            >
