 ...
 * Opens the current diagram via the window.opener if one exists.
 */
EditorUi.prototype.openHis = function(hisId)
{
	// Cross-domain window access is not allowed in FF, so if we
	// were opened from another domain then this will fail.
	if (!hisId) {
	...
	fetch(OPEN_URL + '/' + floor + '/' + hisId, {"method": "get", "headers": {"Authorization": window.authorization}}).then(function(res) {
		if(res.ok) {
			return res.text();
		} else {
			mxUtils.error(mxResources.get('openHisError'), 300, true);
		}
	}).then(function(data){
		...
		// Fires as the last step if no file was loaded
		that.editor.graph.view.validate();
		
		// Required only in special cases where an initial file is opened
		// and the minimumGraphSize changes and CSS must be updated.
		that.editor.graph.sizeDidChange();
		that.editor.fireEvent(new mxEventObject('resetGraphView'));
	}).catch(e => {
	});
		...
		try
		{
			...
			else
			{
				if (xml.length < MAX_REQUEST_SIZE)
				{
					...
					var floor = urlParams['floor'] || "";
					floor = isNaN(floor) ? "" : floor;
					var params = 'fileName=' + encodeURIComponent(name) + '&xml=' + encodeURIComponent(xml) + '&floorId=' + floor
						+ "&width=" + width + "&height=" + height;
					var div = document.createElement('div');
					div.setAttribute('align', 'center');
					var h3 = document.createElement('h3');
					mxUtils.write(h3, mxResources.get('savePending'));
					div.appendChild(h3);
					this.showDialog(div, 420, 90, true, false);
					fetch(SAVE_URL, {"method": "post", "headers": {'Content-type': 'application/x-www-form-urlencoded', "Authorization": window.authorization}
						, "body": params}).then(function(res) {
						return res.json();
					}).then((data) => {
						this.hideDialog();
						if(data.status && data.status !== 200) {
							mxUtils.error(mxResources.get('saveError') + data.message, 400, true);
						} else {
							ipcRenderer.send("syncDinerTable", {"token": window.authorization});
							ipcRenderer.send("syncFloor", {"token": window.authorization});
							var cells = that.editor.graph.getChildCells();
							if (data.result) {
								for (var i = 0; i < cells.length; i++) {
									if (data.result[cells[i].id]) {
										cells[i].setAttribute('tableId', data.result[cells[i].id]);
									}
								}
							}
						}
					}).catch(function(e) {
						mxUtils.error(mxResources.get('saveError') + e.message, 200, true);
					});
				}
				else
				{
