...
import md5 from 'md5';
import { HomePage, PaySetting } from '../utils/constants'
...
const initialState  = {
    ...
    formData: {merchantProperty: {value: 1}, merchantName: {value: ""}, merchantLogo: {value: null}, address: {value: ""}, remark: {value: ""}, businessLicenceNo: {value: null}, yyzzs: {value: null}
    ...
    imageHis: [],//图片变更
    changeAccountLoading: false,//切换子账号loading
}
...
const homePageHandler = {
    ...
    [HomePage.SELECT_CURRENT_MERCHANT_SUCCESS]: (state, action) => {
        ...
        if(currMerchantInfo.logoPath) {
            new_merchantLogo.push({uid: currMerchantInfo.id, name: 'logo', status: 'done', url: `${staticHost}${currMerchantInfo.logoPath}?t=${new Date().getTime()}`});
        }
        ...
        const formData = {merchantProperty: {value: currMerchantInfo.merchantProperty}, merchantName: {value: currMerchantInfo.merchantName}, address: {value: currMerchantInfo.address},
        ...
        //这里是修改基本信息判断表单是否有更改
        let formData_clone = {merchantProperty: currMerchantInfo.merchantProperty, merchantName: currMerchantInfo.merchantName, address: currMerchantInfo.address, 
            remark: currMerchantInfo.remark, merchantLogo: new_merchantLogo, photo: n_fileList};
        const formData_md5 = md5(JSON.stringify(formData_clone));
        return Object.assign({}, state, {loadingCurrMerchantInfo: false, currMerchantInfo, stepVisible, 
            ...
            formData_md5, commitChangeDsiabled: true}); 
    },
    ...
    [HomePage.CREATE_MERCHANTINFO_FORM_FIELD_CHANGE]: (state, action) => {
        ...
        //这里是修改基本信息判断表单是否有更改
        let formData_clone = {merchantProperty: formData.merchantProperty.value, merchantName: formData.merchantName.value, address: formData.address.value, 
            remark: formData.remark.value, merchantLogo: formData.merchantLogo.value, photo: formData.photo.value};
        const new_formData_md5 = md5(JSON.stringify(formData_clone));
        const flag = (new_formData_md5==state.formData_md5);
        return Object.assign({}, state, { formData, commitChangeDsiabled: flag });
    },
    ...
    [HomePage.MERCHANTUSER_REMOVE_IMAGE]: (state, action) => {
        if(action.payload.type == 'yyzz') {
            ...
            //这里是修改基本信息判断表单是否有更改
            let formData_clone = {merchantProperty: new_formData.merchantProperty.value, merchantName: new_formData.merchantName.value, address: new_formData.address.value, 
                remark: new_formData.remark.value, merchantLogo: new_formData.merchantLogo.value, photo: new_formData.photo.value};
            let new_formData_md5 = md5(JSON.stringify(formData_clone));
            let flag = (new_formData_md5==state.formData_md5);
            return Object.assign({}, state, { formData: new_formData, commitChangeDsiabled: flag  });    
        } else {
            ...
            return Object.assign({}, state, { formData: new_formData, photoList: new_photoList, commitChangeDsiabled: flag  });    
        }
    },
    ...
    [PaySetting.SELECT_PAY_STEUP_SUCCESS]: (state, action) => {
        ...
        const new_currMerchantInfo = {...state.currMerchantInfo, alipaySteup: alipaySteup};
        return Object.assign({}, state, {currMerchantInfo: new_currMerchantInfo});
    },
    //切换子账号
    [HomePage.CHANGE_ACCOUNT_PENDING]: (state, action) => {        
        return Object.assign({}, state, { changeAccountLoading: true });
    },
    [HomePage.CHANGE_ACCOUNT_SUCCESS]: (state, action) => {        
        return Object.assign({}, state, { changeAccountLoading: false });
    },
    [HomePage.CHANGE_ACCOUNT_FAILURE]: (state, action) => {        
        return Object.assign({}, state, { changeAccountLoading: false });
    },
}
