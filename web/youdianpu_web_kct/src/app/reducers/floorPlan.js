...
const initialState = {
    ...
    dinersFormData: {dinersNum: {value: ""}, seqNumber: {value: ""}}, //用餐人数formdata
...
const floorPlanHandler = {
    ...
    [FloorPlan.ON_DINERSFORM_FIELD_RESET]: (state, action) => {
        const tableCode = action.payload;
        const dinersFormData = {dinersNum: {value: state.dinersNumMap[tableCode]}, seqNumber: {value: ""}}; //用餐人数formdata
    ...
    [FloorPlan.FINISHED_ORDER_SUCCESS]: (state, action) => {
        ...
        if(tableCodes && tableCodes.length > 0) {
            ...
            tableCodes.forEach(item => {
                if(state.orderListMap[item]) {
                    const new_map = state.orderListMap[item].map(order => order.orderNo != orderNo);
                    tablesOrderMap[item] = new_map;
                }
            });
            ...
            const orderList = state.orderList.filter(item => item.orderNo != orderNo);
    ...
    //重置收银金额
    [FloorPlan.ON_PAYMENT_RECEIVEDAMOUNT_FIELDS_RESET]: (state, action) => {
        const receivedAmount = action.payload;
        return Object.assign({}, state, {receivedAmount: receivedAmount});
    },
}
