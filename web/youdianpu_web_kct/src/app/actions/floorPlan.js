...
const submitOrderKCT = (cart) => {
    /* return (dispatch, getState) => {
        dispatch({type: FloorPlan.SUBMIT_ORDER_PENDING});
        return requestapi({uri: `/api/order/submitKCT`, fetchParams: {body : cart}}).then((data) => {
    ...
    return (dispatch, getState) => {
        dispatch({type: FloorPlan.SUBMIT_ORDER_PENDING});
        return axios.post(`${Config.apiHost}/api/order/submitKCT`, cart, {
            headers,
            timeout: 15000,
        }).then(response => {
 ...
 * @param {*} tableCode 
 */
const listNoCompleteOrderByTableCodeKCT = (tableCode) => {
    return (dispatch, getState) => {
        dispatch({type: FloorPlan.LIST_NOCOMPLETE_ORDER_BY_TABLECODE_PENDING});
        return requestapi({uri: `/api/order/listNoCompleteOrderByTableCodeKCT`, fetchParams: {body : {tableCode}}}).then((data) => {
...
//收银实收金额
const dispatchReceivedAmount = (receivedAmount) => {
    return (dispatch, getState) => {
        return Promise.resolve(
            dispatch({type: FloorPlan.ON_PAYMENT_RECEIVEDAMOUNT_FIELDS_RESET, payload: receivedAmount})
        );
    }
}
...
export default {
    ...
    listNoCompleteOrderByTableCodeKCT,
    dispatch_QRCode,
    deleteOrderItem,
    cancelOrderItem,
    shippedOrderItem,
