...
const onNameChange = (value) => {
    return (dispatch, getState) => {
        return Promise.resolve(
            dispatch({ type: UserRole.USERROLE_ON_NAME_CHANGE, payload: value })
        );
    }
}
...
const search  = (key) => {
    return (dispatch, getState) => {
        dispatch({ type: UserRole.USERROLE_SEARCH_PENDING });
        return requestapi({uri: `/api/userRole/search`, fetchParams: {
            method : 'POST',
            headers: {
            ...
            dispatch({type: UserRole.USERROLE_SEARCH_SUCCESS, payload: data});
        }).catch(err => {
            dispatch({ type: UserRole.USERROLE_SEARCH_FAILURE, payload: err.message });
            throw err;
        });
    }
}
