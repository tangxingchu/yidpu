...
export default class OrderDetail extends Component {
    ...
    render() {
        ...
        const isDisabledFinishedBtn = (currOrder) => {
        ...
        const isDisabledPaymentBtn = (currOrder) => {
        ...
        const isDisabledMemberPaymentBtn = (currOrder) => {
        ...
        const renderHeader = (order, index) => {
            return (
                <div className={styles.orderHeader}>
                    <div><span style={{fontWeight: 'bold'}}>{index+1}</span>{
                        order.seqNumber ? <span>&nbsp;&nbsp;餐牌号:<span style={{fontWeight: 'bold'}}>{order.seqNumber}</span></span> 
                        : null}&nbsp;&nbsp;桌台编号:<span style={{fontWeight: 'bold'}}>{order.tableCode}</span>&nbsp;&nbsp;下单时间:{order.orderTime}</div>
                ...
                <Fragment>
                    ...
                    }
                    {
                        couponConsumeMap[order.orderNo] ? couponConsumeMap[order.orderNo].map(coupon => {
                            return (
                                <div key={coupon.id} className={styles.bottomItem_border}>
                    ...
                    <div className={`${styles.bottomItem_border}`}>
                        合计应收：
                    ...
                    <div style={{display: 'flex', justifyContent: 'flex-end', marginTop: 4}}>
                        <Popconfirm key="finished" title="确认完成该笔用餐订单吗?" onConfirm={() => this.props.finishedOrder(order.orderNo)}>
                            <Button style={{marginRight: 8,}} type="primary" loading={finishedOrderLoading} disabled={isDisabledFinishedBtn(order)}>
                                完成订单
                            </Button>
                        </Popconfirm>
                        <Button key="payment" style={{marginRight: 8,}}  onClick={() => {handlePaymentModalVisible(true, order.orderNo)}} type="primary" disabled={isDisabledPaymentBtn(order)}>收银</Button>
                        <Button key="memberPayment" onClick={() => {handleMemberPaymentModalVisible(true, order.orderNo)}} type="primary" disabled={isDisabledMemberPaymentBtn(order)}>会员消费</Button>
                    </div>
                </Fragment>
            )
        }
        ...
        return (
            <Modal
                ...
                footer={[
                    <Button key="close" onClick={() => handleModalVisible(false)}>
                        关闭
                    </Button>
                ]}
                visible={visible}
