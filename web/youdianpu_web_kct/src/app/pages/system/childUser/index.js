...
import { Card, Row, Col, Button, message, Divider, Table, Popconfirm, Tooltip, Popover, Icon, Modal } from 'antd';
...
class ChildUser extends Component {
    ...
    update = (id, enabled) => {
        ...
        this.props.childUserActions.update({ id, enabled, merchantUsername: this.getRealSub(), merchantId: getUid() }).then(() => {
    ...
    resetPWD = (account) => {
        this.props.childUserActions.resetPWD(account).then((password) => {
            Modal.success({
                title: '密码重置成功',
                content: `新密码是：${password}`,
            });
        });
    }
            ...
            },
            {
                ...
                render: (text, record) => (
                    <Fragment>
                        ...
                        <Divider type="vertical" />
                        <Popconfirm title="确定重置密码吗？" okText="确定" cancelText="取消" onConfirm={() => { this.resetPWD(record.account) }}>
