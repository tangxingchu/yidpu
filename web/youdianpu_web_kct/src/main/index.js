...
const { closeSocketIo } = require('./components/socketServer');
const { initListeners, removeAllListeners } = require('./components/ipcMainHandler');
const package = require("../../package.json");
// const feedUrl = process.env.NODE_ENV === 'development' ? `http://static.yidpu.com/main_updater` : `http://download.yidpu.com`; // 更新包位置
const feedUrl = `http://download.yidpu.com/main_kct`; // 更新包位置
//初始化数据库
let mainWindow, tray;
function createTray() {
    ...
    const contextMenu = Menu.buildFromTemplate([
        { label: '退出', click: () => {
            removeAllListeners();
            closeSocketIo();//关闭socket服务
            mainWindow.destroy(); 
            mainWindow = null;
            app.quit();
        } },
    ])
    tray.setToolTip("一点谱餐饮平台");
...
function createWindow() {
    ...
    initListeners(mainWindow, package.version);
    ...
    const template = [
        ...
        },
        {
            ...
            submenu: [
                ...
                /* {
                    label: '淘宝店铺官网',
                    click() { require('electron').shell.openExternal('www.taobao.com') }
                }, */
                {
                    label: '在线技术客服',
