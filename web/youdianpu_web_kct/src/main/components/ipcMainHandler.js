...
const initListeners = (mainWindow, version) => {
    ...
    ipcMain.on('updateGoods', function (event, arg) {
        var param = [];
        var updateSql = "update goods set name=?, piny=?, unit=?, cost_price=?, price=?, inventory=?, category=?, description=?,";
        param.push(arg.name);
        param.push(arg.piny);
        param.push(arg.unit);
    ...
    ipcMain.on('openFloorPlan', function (event, arg) {
        const { id } = arg;
        const url = process.env.NODE_ENV === 'development' ? `http://localhost:3015/` : `http://localhost:30105/`;
    ...
    ipcMain.on("printOrder", function (event, arg) {
        ...
        if(env === "development") {//使用USB打印机
            ...
            printSettingService.selectPrint(2).then(data => {
                if(data) {
                    printSettingService.printOrder(arg.cart, data.print_ip, data.print_port)
                        .then(() => {
                            event.sender.send("printOrder-reply", {success: true});
                    }).catch(e => {
                        event.sender.send("printOrder-reply", {success: false, message: e.message});
    ...
    ipcMain.on("ping", function (event, arg) {
        ping.sys.probe(arg.printIp, function(isAlive){
            event.sender.send("ping-reply", {isAlive: isAlive});
        });
    });
    //收银流水界面-打印收银小票
    ...
    ipcMain.on("resetNetworkPrinter", function (event, arg) {
        printSettingService.selectPrint(2).then(data => {
            if(data) {
                printSettingService.resetNetworkPrinter(data.print_ip, data.print_port)
                    .then(() => {
    ...
    //快餐厅移动支付之后自动后厨打印
    ipcMain.on("autoPrintOrder", function (event, arg) {
        configService.selectOrderPrintConfig().then(data => {
            if(data && data.config_value == "1") {
                printSettingService.selectPrint(2).then(printerData => {
                    if(printerData) {
                        orderService.selectOrderByOrderNo(arg.orderNos[0], arg.token).then(data => {
                            const cart = {
                            ...
                            printSettingService.printOrder(cart, printerData.print_ip, printerData.print_port)
                                .then(() => {
                                    event.sender.send("autoPrintOrder-reply", {success: true});
                            }).catch(e => {
                                event.sender.send("autoPrintOrder-reply", {success: false, message: e.message});
                            });
                        }).catch(e => {
                            event.sender.send("autoPrintOrder-reply", {success: false, message: e.message});
                        });
                    } else {
                        event.sender.send("autoPrintOrder-reply", {success: false, message: '您还没有设置好网络打印机'});
                    }
                });
            }
        });
