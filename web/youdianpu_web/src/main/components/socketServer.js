            ...
            if (!bl) {
                ...
                io.sockets.on('connection', (socket) => {
                    ...
                    if (clientType == 1) {
                        ...
                        //刷新token
                        socket.on("refreshToken", (token) => {
                            socket.broadcast.emit("refreshTokenMsg", token);
                        });
                        ...
                        socket.on('submitOrderOffline', (arg) => {
                        ...
                        //确定购物车提交订单至服务器
                        socket.on('submitOrderToServer', (arg) => {
                            cacheService.getItem('token').then(data => {
                                placeOrderService.submitOrderToServer(arg, data.value).then((data) => {
                                    socket.emit("submitOrderToServerMsg", {success: true, results: data, message: ""});
                                }).catch(e => {
                                    let fetchError = false;
                                    if(e.message && e.message.toUpperCase() == "FAILED TO FETCH") {
                                        fetchError = true;
                                    } else if(e.message && e.message.toUpperCase().indexOf("TIMEOUT") > -1) {
                                        fetchError = true;
                                    }
                                    socket.emit("submitOrderToServerMsg", {success: false, message: e.message, fetchError});
                                });
                            }).catch(e => {
                                socket.emit("submitOrderToServerMsg", {success: false, message: e.message});
                            });
                        });
                        //下单或确认订单之后打印
                            ...
                            goodsService.selectGoods({}).then(goodsList => {
                                printSettingService.selectPrint(2).then(data => {
                                    if(data) {
                                        ...
                                        socket.emit('app_printOrderMsg', {success: false, tableCode: arg.tableCode, message: '还没有设置好网络打印机'});
                                        //广播给管理员房间 后厨打印失败
                                        socket.to(superRoom).broadcast.emit('printOrderErrorMsg', {success: false, tableCode: arg.tableCode, message: '还没有设置好网络打印机'});
                                    }
                                });
                    ...
                    //点餐机查询每日特价
                    socket.on('listTodayGoodsDays', () => {
                        ruleService.listTodayGoodsDays().then(data => {
                            socket.emit("listTodayGoodsDaysMsg", data);
                        });
                    });
                    //点餐机查询每日折扣
                    socket.on('listEffectiveGoodsDiscount', () => {
                        ruleService.listEffectiveGoodsDiscount().then(data => {
                            socket.emit("listEffectiveGoodsDiscountMsg", data);
                        });
                    });
                    //点餐机查询每日折扣与特价
                    socket.on('listTodayGoodsDaysAndDiscount', () => {
                        ruleService.listTodayGoodsDaysAndDiscount().then(data => {
                            socket.emit("listTodayGoodsDaysAndDiscountMsg", data);
                        });
                    });
                    socket.on('getToken', () => {
                        cacheService.getItem('token').then(data => {
                            socket.emit("getTokenMsg", data.value);
                        });
                    });
                    socket.on('saveLocalCart', (arg) => {
                        localCartService.save(arg.tableCode, JSON.stringify(arg));
                    });
                    socket.on('selectLocalCart', (arg) => {
                        localCartService.selectByTableCode(arg).then(data => {
                            socket.emit("selectLocalCartMsg", data);
                        });
                    });
                    socket.on('deleteLocalCart', (arg) => {
