        ...
        db.sql(`select * from print_setting where print_type=?`, [printSetting.printType], 'get').then(data => {
            if (data) { //更新
                ...
                return db.sql(`insert into print_setting(id, print_type, print_vid, print_pid, print_ip, print_port) values (?, ?, ?, ?, ?, ?)`,
                    [printSetting.id, printSetting.printType, printSetting.printVid, printSetting.printPid, 
                        printSetting.printIp, printSetting.printPort]);
            }
        });
    } else {
        db.sql(`select * from print_setting where id=?`, [printSetting.id], 'get').then(data => {
            if(data) {
                return db.sql(`update print_setting set print_ip=?, print_port=?, name=? where id=?`, 
                        [printSetting.printIp, printSetting.printPort, printSetting.name, printSetting.id]);
            } else {
                return db.sql(`insert into print_setting(id, print_type, print_vid, print_pid, print_ip, print_port, name) values (?, ?, ?, ?, ?, ?, ?)`,
                        [printSetting.id, printSetting.printType, printSetting.printVid, printSetting.printPid, 
                            printSetting.printIp, printSetting.printPort, printSetting.name]);
            }
        });
    }
...
const insert = (printSetting) => {
    return db.sql(`insert into print_setting(id, print_type, print_vid, print_pid, print_ip, print_port, name) values (?, ?, ?, ?, ?, ?, ?)`,
                [printSetting.id, printSetting.printType, printSetting.printVid, printSetting.printPid, printSetting.printIp, 
                    printSetting.printPort, printSetting.name]);
}

const delPrinter = (id) => {
    return db.sql(`delete from print_setting where id=?`, [id]);
}

const selectPrint = (printType) => {
    return db.sql(`select * from print_setting where print_type=?`, [printType], 'all');
}
