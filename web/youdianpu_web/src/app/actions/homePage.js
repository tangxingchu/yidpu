...
//切换子账号
const changeAccount = (username, password) => {
    return (dispatch, getState) => {
        dispatch({type: HomePage.CHANGE_ACCOUNT_PENDING})
        return requestapi({uri: `/api/changeAccount`, fetchParams: {method: 'post', 
                headers: {'Content-type': 'application/x-www-form-urlencoded'}, 
            body: `username=${username}&password=${password}`}}).then(token => {
            window.localStorage.setItem("Authorization", token);
            dispatch({type: HomePage.CHANGE_ACCOUNT_SUCCESS});
        }).catch(err => {
            dispatch({type: HomePage.CHANGE_ACCOUNT_FAILURE, payload: err.message});
            throw err;
        });
    }
}
