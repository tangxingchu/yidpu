...
import { Modal, Affix, Radio, message, notification, Collapse, Tag, Tooltip, Button } from 'antd';
...
class FloorPlan extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            offlinePaymentModalVisible: false,//离线单收银
            currentSubtracts: [],
    ...
    componentDidMount() {
        ...
        ipcRenderer.on('initPlaceOrder-reply', (event, arg) => {           
            this.props.floorPlanActions.dispatchInit_local(arg).then(() => {
                ...
                //当前有效并且符合条件的减免规则
                if(local_cartList && local_cartList.length > 0) {
                    let totalPrice = 0;
                    local_cartList.forEach(item => {
                        totalPrice += item.itemPrice;
                    });
                    let orderTime = local_cartList[0].createTime;
                    ipcRenderer.on('listCurrentGoodsSubtract-reply', (event, arg) => {
                        ipcRenderer.removeAllListeners("listCurrentGoodsSubtract-reply");
                        this.setState({currentSubtracts: arg});
                    });
                    ipcRenderer.send("listCurrentGoodsSubtract", {totalPrice, orderTime});
                }
            });
        });
        ...
        this.editor.addAction('viewOrderItem', (editor) => {
            ...
            ipcRenderer.on("selectLocalCart-reply", (event, arg) => {
                if(arg && arg.length > 0) {
                    ipcRenderer.send('initPlaceOrder', { tableCode: this.tableCode });
                    this.handleOfflinePaymentModalVisible(true);
                } else {
                    this.props.floorPlanActions.listNoCompleteOrderByTableCode(tableCode);
                    this.props.floorPlanActions.handleOrderDetailVisible(true);
                }
 ...
 */
    ...
    handleOfflinePaymentModalVisible = (flag) => {
        this.setState({offlinePaymentModalVisible: !!flag});
    }
    ...
    submitOrder = (dinersNum) => {
        ...
        let cart = {orderMethod: 3, cartItem: local_cartList, tableCode: this.tableCode, tableStatus: tableStatusObj.status, dinersNum: dinersNum};
        if(this.offlineModel) {
            //将订单保存在本地
            ipcRenderer.send('saveLocalCart', cart);
            //只需要追加的单
            let new_local_cartList = cart.cartItem.filter(item => item.isPrinter == "0");
            let new_cart = {...cart, cartItem: new_local_cartList};
            this.setState({ placeOrderModalVisible: false, dinersNumModalVisible: false });
            //提交至本地的一些处理(修改桌台状态, 减库存)
            ipcRenderer.send('submitOrderOffline', {...new_cart, createUser: getSub().indexOf(':') > -1 ? getSub().split(":")[1] : getSub()});
            ...
            this.printTicketAndOrder({...new_cart, orderNo: "离线单"});
        } else {
            this.props.floorPlanActions.submitOrder(cart).then((results) => {
                ...
                if(e.message && (e.message.toUpperCase().indexOf("TIMEOUT") > -1
                    ...
                    Modal.confirm({
                        title: '网络出现了故障,是否后厨打单并暂时将订单保存在本地？',
                        content: '',
                        okText: "是",
                        onCancel: "否",
                        onOk: () => {
                            ...
                            this.setState({ placeOrderModalVisible: false, dinersNumModalVisible: false });
                            //提交至本地的一些处理(修改桌台状态, 减库存)
                            ipcRenderer.send('submitOrderOffline', {...cart, createUser: getSub().indexOf(':') > -1 ? getSub().split(":")[1] : getSub()});
                            ipcRenderer.send('udpateTableStatus', {tableCode: this.tableCode, status: 4});
                            //更改cell样式(桌台状态=4)
                            this.props.floorPlanActions.changeTableStatus({tableCode: this.tableCode, status: 4});
                            this.printTicketAndOrder({...cart, orderNo: "离线单"});
                        },
                        onCancel: () => {
    ...
    orderGathering = (payMenthod, remark) => {
        const { orderList, receivedAmount } = this.props.floorPlan;
        const { currMerchantInfo } = this.props.homePage;
        if(orderList.length > 0) {
            this.props.floorPlanActions.orderGathering(orderList[0].orderNo, payMenthod, receivedAmount, remark).then((orderNos) => {
    ...
    relateFrontOrder = (selectedRowKeys, remark) => {
        ...
        this.props.floorPlanActions.relateFrontOrder(currOrder.orderNo, selectedRowKeys, remark).then((orderNos) => {
            message.success("收银成功");
        ...
        ipcRenderer.on('selectPrintConfigs-reply', (event, arg) => {
            if(arg) {
                ...
                if(autoPrintTicket && autoPrintOrder) {
                    ...
                    ipcRenderer.send('printTicket', {cart, merchant: currMerchantInfo});
                } else if(autoPrintOrder && !autoPrintTicket) {
                    //后厨打印用餐订单明细
                    ipcRenderer.on('printOrder-reply', (event, arg) => {
                        if(arg.success) {
                            //修改订单打印状态
    ...
    render() {
        const { modalVisible, placeOrderModalVisible, copyPlaceOrderModalVisible, extraModalVisible, QRCodeModalVisible, couponConsumeVisible, 
            ...
            printOrderModalVisible, changeTableModalVisible, offlinePaymentModalVisible, currentSubtracts
        } = this.state;//弹出modal框
        ...
        return (
            <div className={styles.container} ref={(node) => { this.container = node; }} >
                ...
                <OfflinePaymentModal visible={offlinePaymentModalVisible}
                    handleModalVisible={this.handleOfflinePaymentModalVisible}
                    tableCode={this.tableCode}
                    dataSource={local_cartList}
                    currentSubtracts={currentSubtracts}
                >
                ...
                <Modal
                    ...
                    footer={[
                        ...
                        <Button key={"submit"} type="primary" loading={submitOrderLoading} onClick={() => {
                            ...
                            this.props.floorPlanActions.submitOrder(cart).then(() => {
                                ipcRenderer.send('deleteLocalCart', this.tableCode);
                                ipcRenderer.send('clearShoppingCart', {tableCode: this.tableCode});
                                this.props.floorPlanActions.clearShoppingCart(this.tableCode);
