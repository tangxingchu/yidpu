...
import QRCodeModal from '../../dashboard/floorPlan/qrcodeModal';
import BindModal from './bindModal';
import { memberQRCodeURL } from '../../../common/config';
...
class MemberInfoPage extends Component {
    ...
    handleBindModalVisible = (flag, memberId, phone) => {
        if(flag) {
            this.props.memberInfoActions.dispatch_bind({id: {value: memberId}, phone: {value: phone}});
        }
        this.setState({bindModalVisible: !!flag});
    }
    ...
    componentDidMount() {
        ...
        ipcRenderer.on("callMemberBindMSG", (event, arg) => {
            this.props.memberInfoActions.dispatch_bind({bindType: {value: "" + arg.bindType}, code: {value: arg.code}});
        });
    }

    componentWillUnmount() {
        ipcRenderer.removeAllListeners("callMemberBindMSG");
    }
    ...
    handleQRCodeModalVisible = (flag) => {
        if(flag) {
            ipcRenderer.on('qrcode-reply', (event, arg) => {
                this.props.memberInfoActions.dispatch_QRCode(arg);
                ipcRenderer.removeAllListeners("qrcode-reply");
            });
            ipcRenderer.send('qrcode', `${memberQRCodeURL}/bindRedirect.html?merchantId=${getUid()}`);
        }
    ...
    handleBind = (values) => {
        this.props.memberInfoActions.memberBind(values.id, values.bindType, values.code).then(() => {
            message.success("绑定成功");
            this.handleBindModalVisible(false);
        });
    }

    handleUnbind = (values) => {
        this.props.memberInfoActions.memberunBind(values.id, values.bindType).then(() => {
            message.success("成功解除绑定");
        });
    }

    render() {
        const { changeDescModalVisible, QRCodeModalVisible, bindModalVisible } = this.state;
        const path = this.props.match.path;
        const { history } = this.props;
        const { loading, saveLoading, memberList, memberInfoData, searchFormData, memberDetailData, detailLoading, pageSize, total,
            currentPage, freezeLoading, changeDescValue, qrcodeData, bindData, bindLoading, hideUnbindBtn, unbindLoading } = this.props.memberInfo;
        ...
        const renderContent = () => (
            <span>录入线下采集的会员基本信息，或者一点谱公众号平台引流过来的会员信息转换为您店铺会员。如果您需要单独开发微信公众号采集线上会员信息可以与我们客服联系,我们为您定制开发微信公众号。
                    会员绑定微信或支付宝请让顾客扫此<a href="javascript:void(0)" onClick={() => this.handleQRCodeModalVisible(true)}>二维码</a></span>
        )
        ...
        return (
            ...
            >   
                ...
                <QRCodeModal modalVisible={QRCodeModalVisible}
                    ...
                    message={`点击绑定微信或支付宝，然后让顾客扫此二维码完成绑定。绑定之后会员可直接在一点谱公众号上充值至您微信商户号或支付宝，无需到店充值。`}
                ...
                <BindModal visible={bindModalVisible}
                    ...
                    {...bindData}
                    loading={bindLoading}
                    handleSubmit={this.handleBind}
                    unbind={this.handleUnbind}
                    hideUnbindBtn={hideUnbindBtn}
