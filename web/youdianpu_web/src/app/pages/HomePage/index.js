...
class HomePage extends Component {
	...
	componentDidMount() {
		...
		ipcRenderer.on("customerPlaceOrderMSG", (event, arg) => {
			...
			ipcRenderer.send('udpateTableStatus', {tableCode: arg.tableCode, status: 3});
            this.props.floorPlanActions.changeTableStatus({tableCode: arg.tableCode, status: 3});
		});
		//桌台扫码支付成功消息
		ipcRenderer.on("callPaymentFinishedMSG", (event, arg) => {
			...
			// } else {
				const notificationEntity = {
					title: '收银消息',
					body: `桌台编号${arg.tableCodes}已付款,金额:￥${numeral(arg.payAmount).format('0,0.00')},【顾客桌台扫码付款】。`,
					icon: "/images/yidpu/logo.png",
				}
				const myNotification = new window.Notification(notificationEntity.title, notificationEntity);
				myNotification.onclick = () => {
				...
				ipcRenderer.send("autoPrintCashier", {orderNos: arg.orderNos, token: getToken(), merchant: currMerchantInfo});
			// }
		...
		this.refreshTokenIntv = window.setInterval(() => {
			this.props.homePageActions.refreshToken().then(token => {
				...
				//发给其他客户端token
				// ipcRenderer.send('refreshToken', token);
			});
		}, 1000 * 60 * 60 * 4);
	}
