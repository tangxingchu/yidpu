...
export default class DaysSetting extends Component {
    ...
    render() {
        const { modalVisible, handleModalVisible, goodsList, goodsCheckedKeys, goodsCheckedRows, onTableChange,
            current, goodsDayWeekName, goodsDayWeek, onEffectiveTimeChange, onExpiredTimeChange, onPriceChange, onLimitChange,
            saveGoodsDay, saveGoodsDayLoading, goodsDayList } = this.props;
        const goodsDayFilterList = goodsDayList.filter(item => item.week == goodsDayWeek);
        const steps = [{
            key: "1",
            title: '选择商品',
            content: <GoodsTable goodsList={goodsList}
                ...
                goodsFilterList={goodsDayFilterList}
            />,
        }, {
            key: "2",
            title: '设定价格',
            content: <SetPrice selectedGoodsData={goodsCheckedRows}
        ...
        const renderFooter = () => {
            ...
            if (current === steps.length - 2) {
                dynamicBtn.push(<Button key="comfirm" type="primary" loading={saveGoodsDayLoading} onClick={() => saveGoodsDay()}>确定</Button>);
            }
            return [
        ...
        const extraTitle = (
            <div>
                配置[{goodsDayWeekName}]特价商品
                <span className={styles.tooltip}>(注意: 一天中不能设置相同的商品)</span>
            </div>
        )
        return (
            <Modal
...
class SetPrice extends Component {
    ...
    render() {
        ...
        const { onEffectiveTimeChange, onExpiredTimeChange, onPriceChange, onLimitChange } = this.props;
        ...
        return (
            <Fragment>
                <div className={styles.stepPrice}>
                    <Alert
                        showIcon
                        message="特价商品设置好之后无法再修改价格,只能删除后重新添加。"
                        style={{ marginTop: 24 }}
                    />
                        ...
                        selectedGoodsData.map((goods, index) => {
                            return (
                                <div className={styles.goodsItem} key={goods.id}>
                                    ...
                                    特价：<InputNumber defaultValue={1} onChange={(value) => onPriceChange(goods.id, value)} min={1} max={goods.price} size="large" />
                                    每桌限点<InputNumber defaultValue={1} onChange={(value) => onLimitChange(goods.id, value)} min={1} max={goods.price} size="large" />{goods.unitName}
                                </div>
                            )
                        })
                    }
                    ...
                    selectedGoodsData.map(goods => {
                        return (
                            <div key={goods.id} className={styles.goods}>
                                ...
                                <div>特价:<span className={styles.goods_newprice}>￥{numeral(goods.newPrice).format('0,0.00')}</span></div>
                            </div>
                        )
                    })
                }
